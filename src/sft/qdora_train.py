...
"""Train a QDoRA SFT baseline for disruption classification.

This script is intentionally self-contained. It:
1. Loads the in-repo JSONL dataset and split manifest.
2. Builds an RL-aligned JSON output contract for SFT.
3. Fine-tunes a 4-bit base model with DoRA adapters.
4. Runs held-out generation eval and writes metrics/artifacts.
...
def default_dataset_path() -> Path:
    return repo_root() / "data" / "sci_balanced_from2m_no_ovr.rl_balanced.jsonl"


def default_splits_path() -> Path:
    return repo_root() / "data" / "sci_balanced_from2m_no_ovr.splits.json"
...
def main() -> None:
    ...
    raw_train_examples = build_sft_examples(
        train_records,
        tokenizer,
        args.teacher_confidence_mode,
    )
    raw_val_examples = build_sft_examples(
        val_records,
        tokenizer,
        args.teacher_confidence_mode,
    )
    raw_test_examples = build_sft_examples(
        test_records,
        tokenizer,
        args.teacher_confidence_mode,
    )

    train_examples, train_filter_stats = filter_examples_by_length(
        tokenizer,
        raw_train_examples,
        args.max_length,
    )
    val_examples, val_filter_stats = filter_examples_by_length(
        tokenizer,
        raw_val_examples,
        args.max_length,
    )
    test_examples, test_filter_stats = filter_examples_by_length(
        tokenizer,
        raw_test_examples,
        args.max_length,
    )
    ...
    if not train_examples:
        raise ValueError("No train examples remain after max-length filtering.")
    if not test_examples:
        raise ValueError("No test examples remain after max-length filtering.")
